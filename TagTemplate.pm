...
#
# Text::TagTemplate
# A simple template parsing system using XML-like "shortcut" tags
#
...
sub version { $Text::TagTemplate::VERSION }
...
sub new {
    ...
    my $format = "<!-- %s -->\n";
	# Default debugging code output mode is off
	my $debug = 0;
    ...
    while ($opt = shift) {
		...
		if ($opt eq "debug") {
			# 22.04.00(tw): debugging mode switch
			$debug = 1 if (shift == 1);
		}
    }
...
sub recursive_interpret{
	...
	} elsif ( $tokenMatch = $self->match_ifdefined($token) ) {
		# added 25.04.00(tw)		
	    # First we have to find the ending </ifdefined> tag
	    # Right now this happens by joining then splitting
	    $text = join '', @tokens;
	    my @context = split /(<\/?ifdefined[^>]*>)/iso, $text;
	    my $context;
	    ($context, @context) =  $self->context('ifdefined', @context);
        	# Perform the ifdefined directive
	    $self->do_ifdefined($context, $data, $tokenMatch);
            # Restore token list at the point *after* the </ifdefined>
	    $text = join '', @context;
	    @tokens = $self->tokenize($text);

	} elsif ( $tokenMatch = $self->match_ifundefined($token) ) {
		# added 25.04.00(tw)		
	    # First we have to find the ending </ifundefined> tag
	    # Right now this happens by joining then splitting
	    $text = join '', @tokens;
	    my @context = split /(<\/?ifundefined[^>]*>)/iso, $text;
	    my $context;
	    ($context, @context) =  $self->context('ifundefined', @context);
        	# Perform the ifundefined directive
	    $self->do_ifundefined($context, $data, $tokenMatch);
            # Restore token list at the point *after* the </ifundefined>
	    $text = join '', @context;
	    @tokens = $self->tokenize($text);
...
sub tokenize{
    ...
    split ( m/(
	      ...
	      # with context
	      | <with [^>]*>
	      # ifdefined context 25.04.00(tw)
	      | <ifdefined [^>]*>
	      # ifundefined context	25.04.00(tw)
	      | <ifundefined [^>]*>
	      )/isxo, $text );
}
...
#####
# Match_Variable: Does the token match $(variable) or ${variable} or &variable;
#
...
sub do_variable{
	...
	if ( $variable =~ /^now$/io ) {
	    ...
	    $self->{out} .= $variable if ( $self->{debug} ); # 22.04.00(tw): debugging mode switch
	} #if NOW, DATE, TIME
    }
}
...
sub do_assign{
	...
	$self->{out} .= $token if ( $self->{debug} );	# 22.04.00(tw): debugging mode switch
    }
}
...
sub match_repeat{
    ...
    # 22.04.00(tw): changed \w+ to [\w\/]+ for "top/mid/bot"
...
sub do_repeat{
	...
	#
	    ...
	    $self->{out} .= $text if ( $self->{debug} );	# 22.04.00(tw): debugging mode switch
	}
    } else {
...
#####
# Match_Ifdefined: Does it match a <ifdefined ...> tag?
#
# Pre: Passed the token to check
# Post: Returns the namespace if it matches, undef otherise
sub match_ifdefined{
    ...
    # token must start with <ifdefined ... name="..." ...>
    if ( $token =~ /^<ifdefined.*?\s+name=\"([\w\/]+)\"[^>]*>$/io) {
	return $1;
    } else {
  	return undef;
    }
...
#####	25.04.00(tw)
# Do_Ifdefined: Interpret the text given if the path is defined
#
# Pre: Passed the text to interpret, the data hash and the path from the condition
# Post: Places the recursively-parsed text or an error message into $self->{out}
sub do_ifdefined{
    my ($self, $text, $hash, $path) = @_;

    if ( defined $text ) {
        # resolve the path within the data hash
		my $context = $self->resolve_path($hash, $path);
		
		# condition of 'ifdefined' is true when context is defined
		if ( defined $context ) {
	    	$self->recursive_interpret($text, $hash);
		}
    } else {
	$self->{out} .= $self->message("Ifdefined context left empty.");
	$self->{out} .= $text if ( $self->{debug} );
    }
...
#####
# Match_Ifundefined: Does it match a <ifundefined ...> tag?
#
# Pre: Passed the token to check
# Post: Returns the namespace if it matches, undef otherise
sub match_ifundefined{
    my ($self, $token) = @_;
    
    # token must start with <ifundefined ... name="..." ...>
    if ( $token =~ /^<ifundefined.*?\s+name=\"([\w\/]+)\"[^>]*>$/io) {
	return $1;
    } else {
  	return undef;
    }
} # end match_ifundefined


#####	25.04.00(tw)
# Do_Ifundefined: Interpret the text given if the path is defined
#
# Pre: Passed the text to interpret, the data hash and the path from the condition
# Post: Places the recursively-parsed text or an error message into $self->{out}
sub do_ifundefined{
    ...
    if ( defined $text ) {
		...
		# condition of 'ifundefined' is true when context is NOT defined
		if ( !defined $context ) {
	    	$self->recursive_interpret($text, $hash);
		}
    } else {
	$self->{out} .= $self->message("Ifundefined context left empty.");
	$self->{out} .= $text if ( $self->{debug} );
    }
...
#####
# Resolve_path: Resolves the context path in the data structure
#
# Pre: A hash of data and the context path a la XQL/XSL (see sub set_hash)
# Post: Returns a reference to data subset or undefined if path doesn't exist
sub resolve_path {
	my ($self, $hash, $path) = @_;

	# resolve the path within the data hash structure
	# reset context to top of tree if path begins with "/"
    my @tree = split(m:/+:, $path);
    my $context = $tree[0] ? $hash : $self->{top};
	while (@tree and $context and ref($context)) {
        ...
        } elsif (ref($context) eq "HASH" and defined($context->{$el})) {
            $context = $context->{$el};
        } else {
            $context = undef;
            last;
...
sub context {
    ...
    $tag = $1 if $tag =~ m:^\s*</?(\w+):is;
    my $context = "";
    my $sub_lvl = 0;
  WHILE:
    while( @tokens ) {
	my $t = shift @tokens;
	++$sub_lvl if $t =~ m:^\s*<$tag:is;
	if ( $t =~ m:^\s*</$tag>\s*:is ) {
	    if( $sub_lvl ) {
		--$sub_lvl;
	    }else{
...
The Text::TagTemplate module provides for assembling templates using a
simple XML-like tag markup.  It works on any text document, including
(of course) HTML and XML. Previous versions were distributed as
XML::Template, but this was a misnomer because it is not really XML.
...
that it supports more than just variable substituion. Using tag
...
$(variable) or ${variable} or &variable; -- identifiers are word characters
...
<ifdefined name="variable"></ifdefined> -- interpret only if the variable is
defined in the namespace ( = the variable exists with a defined value )

<ifundefined name="variable"></ifundefined> -- interpret if the variable is
undefined in the namespace ( = doesn't exist or has an undefined value )

The syntax for the namespaces specified with the "name" 
attribute of the <with>, <repeat>, <ifdefined> and <ifundefined> tags follow
a syntax similar to XQL/XSL:
